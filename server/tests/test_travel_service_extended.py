...
def test_get_restaurants_with_cuisine(travel_service, mocker):
    ...
    mocker.patch.object(travel_service, '_make_api_request', return_value=mock_response)
...
def test_conversation_state_transitions(travel_service):
    ...
    assert state['state'] == 'START'
    assert "Where would you like to go?" in state['message']
...
def test_get_travel_plan_edge_cases(travel_service, mocker):
    ...
    mocker.patch.object(travel_service, '_get_location_id', return_value="123")
    mocker.patch.object(travel_service, 'get_places', return_value=[
        {'name': 'Place 1', 'category': 'attraction'},
        {'name': 'Place 2', 'category': 'restaurant'}
    ])
    mocker.patch.object(travel_service, 'get_attractions', return_value=[
        {'name': 'Attraction 1', 'category': 'museum'},
        {'name': 'Attraction 2', 'category': 'park'}
    ])
    mocker.patch.object(travel_service, 'get_restaurants', return_value=[
        {'name': 'Restaurant 1', 'category': 'restaurant'},
        {'name': 'Restaurant 2', 'category': 'cafe'}
    ])
...
def test_create_day_activities_edge_cases(travel_service):
    """Test edge cases in day activities creation"""
    places = {
        'attractions': [{'name': 'Museum', 'category': 'museum', 'location': {'lat': 0, 'lng': 0}}],
        'restaurants': [{'name': 'Restaurant', 'category': 'restaurant', 'location': {'lat': 0, 'lng': 0}}],
        'nature': [{'name': 'Park', 'category': 'park', 'location': {'lat': 0, 'lng': 0}}],
        'shopping': [{'name': 'Mall', 'category': 'shopping', 'location': {'lat': 0, 'lng': 0}}],
        'entertainment': [{'name': 'Cinema', 'category': 'entertainment', 'location': {'lat': 0, 'lng': 0}}],
        'cultural': [{'name': 'Temple', 'category': 'temple', 'location': {'lat': 0, 'lng': 0}}]
    }
    ...
    activities = travel_service._create_day_activities(places, "mixed", False)
    assert len(activities) > 0
    assert not any(act['category'] == 'restaurant' for act in activities)
